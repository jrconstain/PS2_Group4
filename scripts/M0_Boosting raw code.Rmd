...
```{r}
# MODELO GRADIENTE BOOSTING TREES  CON AJUSTES EN LOS HIPERPARÁMETROS y balancenado con up
# ESTE APENAS ESTÁ EN CONSTRUCCION
...
ctrl <- trainControl(
  ...
  classProbs = TRUE,           # para probs de la clase positiva
  savePredictions = "final",   # OOF para buscar umbral
  sampling = "up",             # <-- balanceo: sobre-muestreo de la clase minoritaria
  summaryFunction = function(data, lev = NULL, model = NULL) {
    tp <- sum(data$pred == lev[2] & data$obs == lev[2])
    fp <- sum(data$pred == lev[2] & data$obs != lev[2])
    fn <- sum(data$pred != lev[2] & data$obs == lev[2])
  ...
  , verboseIter = TRUE        # (opcional) para ver progreso en consola
)
