...
# Árbol de Clasificación (rpart2) optimizando F1 variables usadas por Juan José en EN
...
# -------------------------------
# 4) Imputación (mediana num / moda factor) aprendida en TRAIN
# -------------------------------
# NaN -> NA
train_df <- train_df %>% mutate(across(where(is.numeric), ~ ifelse(is.nan(.), NA_real_, .)))
test_df  <- test_df  %>% mutate(across(where(is.numeric), ~ ifelse(is.nan(.), NA_real_, .)))
...
get_mode <- function(x){
  ux <- unique(x[!is.na(x)]); if(length(ux)==0) return(NA); ux[which.max(tabulate(match(x, ux)))]
}
...
for(fc in fact_cols){
  if(is.factor(train_imp[[fc]])){
    ...
    v <- train_imp[[fc]]; v[is.na(v)] <- fact_modes[[fc]]; train_imp[[fc]] <- factor(v)
  }
}
for(nc in num_cols){
  train_imp[[nc]][is.na(train_imp[[nc]])] <- num_medians[[nc]]
...
# Aplicar a TEST con los mismos imputadores y alinear niveles
test_imp <- test_df
for(fc in fact_cols){
  if(!is.factor(test_imp[[fc]])) test_imp[[fc]] <- factor(test_imp[[fc]])
  if(!fact_modes[[fc]] %in% levels(test_imp[[fc]]))
