...
# Predicción directa de clases
train_pred <- predict(dt_clasif_F1, newdata = train_use, type = "raw")

# Asegurar que las clases estén en el mismo formato
train_pred <- factor(train_pred, levels = c("no_pobre", "pobre"))
true_labels <- train_use$Pobre

# Calcular métricas
tp <- sum(train_pred == "pobre" & true_labels == "pobre")
fp <- sum(train_pred == "pobre" & true_labels != "pobre")
fn <- sum(train_pred != "pobre" & true_labels == "pobre")
...
train_pred <- predict(tree_ranger_grid, newdata = train_use, type = "raw")
...
roc_obj_en<-roc(response=tree_ranger_grid$pred$obs,  # Valores reales de la variable objetivo
                  ...
                  levels = c("no_pobre", "pobre"),  # # Establece la referencia control y caso (no pobre = negativo, pobre = positivo) 
                  direction = "<")  # "<" significa que "pobre" es positivo
...
# Obtener las probabilidades de la clase "pobre" en el conjunto de entrenamiento
train_prob <- predict(tree_ranger_grid, newdata = train_use, type = "prob")[, "pobre"]

# Umbral de decisión
th <- prec_recall$threshold[which.max(prec_recall$F1)]

# Predicción binaria
train_pred <- ifelse(train_prob >= th, "pobre", "no_pobre")
train_pred <- factor(train_pred, levels = c("no_pobre", "pobre"))
...
# Predicción de probabilidad de pobre
...
##Nueva imputación (IMPORTANTE CORRER NUEVAMENTE LAS BASES Y EL MANEJO DE DATOS HASTA LA IMPUTACIÓN ANTERIOR, DESDE AHÍ CONTINUAR CON ESTE CÓDIGO)
# Numéricas_0 -> mediana
for (v in num_vars) {
...
vars_imp <- c("horasxpropocc", "ingreso_por_activos", "meanedu_x_propocc", "propocc_x_lp", "prop_occ_nper", "P5130", "horasxedu_occ", "Npersug", "edu_occ_lp", "tam_emp_prom_occ", "mean_edu_occ", "Nper", "n_personas", "prom_horas_occ", "propocc_x_prest", "prop_occ_pet", "prestaciones", "prop_prest_occ", "n_no_pet_flex", "edad_jefe2", "edad_prom_occ2", "prop_ina_nper", "cotiza_pens_jefe2", "horas_jefe", "P5140", "prop_occ_pet_flex", "edad_jefe", "antig_prom_occ", "edad_prom_occ", "prop_no_pet_nper", "antig_jefe", "Lp", "tam_emp_jefe", "jefe_max_edu_years", "P5000", "n_occ") #df_vi$feature
