...
```{r}
# MODELO GRADIENT BOOSTING TREE 1  0.70
...
```{r}
# MODELO GRADIENT BOOSTING TREE 2 0.72
...
```{r}
# ==========================================================
# GUARDADO ROBUSTO DE RESULTADOS — GBM (V3 FULL VARS)
# Requiere en memoria:
#   gbm_fit_2, gbm_grid_2, f1_by_thr, best_thr, umbral_opt,
#   kaggle_sub_2, test_prob_2, Pobre_pred_2, train_use, vars, ctrl
# ==========================================================
library(readr); library(dplyr); library(tibble); library(here); library(caret)

# 1) Carpeta con timestamp y etiqueta del experimento
run_tag   <- "GBTREE_F1_V3_FULLVARS"
run_stamp <- format(Sys.time(), "%Y%m%d_%H%M%S")
out_dir   <- here::here("stores", paste0(run_tag, "_", run_stamp))
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

# 2) Guardar MODELO (RDS comprimido)
saveRDS(gbm_fit_2, file = file.path(out_dir, "gbm_fit_2.rds"), compress = "xz")

# 3) Guardar GRID y mejor tuning
write_csv(gbm_grid_2,                 file.path(out_dir, "tune_grid.csv"))
write_csv(as_tibble(gbm_fit_2$bestTune), file.path(out_dir, "best_tune.csv"))

# 4) Resultados de CV (todas las combinaciones)
write_csv(as_tibble(gbm_fit_2$results), file.path(out_dir, "cv_results.csv"))

# 5) OOF del mejor modelo (por savePredictions = 'final')
#    obs: etiqueta real; pred: clase con corte 0.5; 'pobre' = prob clase positiva
oof <- gbm_fit_2$pred %>%
  select(rowIndex, obs, pred, pobre, Resample)
write_csv(oof, file.path(out_dir, "oof_predictions.csv"))

# 6) Curva F1 vs threshold y umbral óptimo
write_csv(f1_by_thr,                     file.path(out_dir, "f1_by_threshold.csv"))
write_csv(as_tibble(best_thr),           file.path(out_dir, "best_threshold_row.csv"))
writeLines(as.character(umbral_opt),     con = file.path(out_dir, "umbral_opt.txt"))

# 7) Matriz de confusión OOF al umbral óptimo (útil para diagnóstico)
levs <- levels(train_use$Pobre)
neg  <- levs[1]; pos <- levs[2]
pred_cls_oof <- factor(ifelse(oof$pobre >= umbral_opt, pos, neg), levels = levs)
cm_oof <- caret::confusionMatrix(pred_cls_oof, factor(oof$obs, levels = levs), positive = pos)
capture.output(cm_oof, file = file.path(out_dir, "confusion_matrix_oof.txt"))

# 8) Importancia de variables
vi <- caret::varImp(gbm_fit_2)$importance %>%
  rownames_to_column("variable") %>% arrange(desc(Overall))
write_csv(vi, file.path(out_dir, "variable_importance.csv"))

# 9) Predicciones de TEST (proba y binaria) + submission
#    id desde test_hogares_enriquecido (ya usaste esto en el submit)
id_test <- as.character(test_hogares_enriquecido$id)
stopifnot(length(id_test) == length(test_prob_2),
          length(id_test) == length(Pobre_pred_2))

pred_test_full <- tibble(
  id         = id_test,
  prob_pobre = as.numeric(test_prob_2),
  Pobre_pred = as.integer(Pobre_pred_2)
)
write_csv(pred_test_full, file.path(out_dir, "test_predictions_full.csv"))

#   Submission final (ya construida arriba como kaggle_sub_2)
write_csv(kaggle_sub_2, file.path(out_dir, "kaggle_submission.csv"))
...
meta <- list(
  ...
  bag.fraction  = 0.5,
  metric        = "F1",
  best_tune     = gbm_fit_2$bestTune,
  grid_rows     = nrow(gbm_grid_2)
)
saveRDS(meta, file = file.path(out_dir, "meta_config.rds"), compress = "xz")
...
# Verifica que test_final tenga exactamente las mismas columnas de `vars`
...
# 5) OOF del bestTune y búsqueda de umbral
...
# 1) Importancia
library(caret)
imp <- varImp(xgb_f1, scale = FALSE)

# 2) Diccionario de etiquetas
var_labels <- c(
  "meanedu_x_propocc" = "Avg years of education (employed) × share employed in household",
  "horasxpropocc"     = "Hours worked × share employed in household",
  "ingreso_por_activos" = "Income from assets (household)",
  "P5130"             = "Imputed/estimated rent (P5130)",
  "propocc_x_prest"   = "Share employed × benefits",
  "Npersug"           = "Number of persons in the spending unit (UG)",
  "tam_emp_prom_occ"  = "Average firm size of the employed",
  "Nper"              = "Number of persons in the household",
  "Lp"                = "Poverty line (Lp)",
  "P5140"             = "Rent paid (P5140)"
)

# 2
old_names <- rownames(imp$importance)
new_names <- ifelse(old_names %in% names(var_labels), var_labels[old_names], old_names)
rownames(imp$importance) <- new_names

# 3) Grafica
plot(imp, top = 10)

# ruta
out <- here::here("views", "xgb_importance_top10.png")

png(filename = out, width = 1600, height = 900, res = 150)
plot(imp, top = 10)                                         
