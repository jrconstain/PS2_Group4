...
```{r}
# 1) Importancia "clásica" de caret (ranking único)
library(caret)
...
imp_df <- imp$importance %>%
  as.data.frame() %>%
  rownames_to_column("variable") %>%
  arrange(dplyr::desc(Overall))    # si es binaria tendrás columna "Overall"

write_csv(imp_df, here("stores", "xgboost_importancia_varImp.csv"))
...
# 2) Diccionario de etiquetas (edítalo a tu gusto)
# 1) Crea el diccionario con otro nombre
var_labels <- c(
...
# 2) Reemplaza los rownames con fallback al original
...
png(filename = out, width = 1600, height = 900, res = 150)  # tamaño y DPI
plot(imp, top = 10)                                         # tu plot
