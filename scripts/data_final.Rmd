...
agg_personas_to_hogares_plus_2 <- function(df_personas) {
  ...
  df_personas %>%
    ...
    summarise(
      ...
      jefe_max_edu_years = ifelse(any(P6050 == 1),
                                  max(edu_years[P6050 == 1], na.rm = TRUE),
                                  NA_real_),                   # NUEVA (años)
      ...
      desoc_con_ingresos_cat = dplyr::case_when(
...
write_csv(df_train_hogares, here::here("stores", "train_hogares_enriquecido_final.csv"))
write_csv(df_test_hogares,  here::here("stores", "test_hogares_enriquecido_final.csv"))
```
...
train_hogares_enriquecido <- train_hogares_enriquecido %>%
  mutate(
    ...
    horasxpropocc    = horas_prom_occ * prop_occ_nper,
...
# Categóricas
cat_vars <- c(
  "Depto","P5000","P5010","P5090","P5100","P5130","P5140",
