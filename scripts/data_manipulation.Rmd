...
#Desbalanceo
ggplot(df_train_hogares,aes(x = Pobre, y = after_stat(count / sum(count)), fill = Pobre)) +
  geom_bar() +
  theme_minimal() +
  scale_y_continuous(labels = scales::percent_format(), breaks = seq(0, 1, by = 0.05)) +  
  scale_fill_manual(values = c("1" = "orange", "0"= "blue")) + 
   labs(x = "", y = "%")  
#Tenemos un fuerte desbalanceo de la categoria pobres en el training
