...
```{r}
# Function to compute a household-level feature:
# mean years of education among the occupied persons in each household
agg_mean_edu_occupied <- function(df_personas) {
  df_personas %>%
    dplyr::group_by(id) %>%
    dplyr::summarise(
...
train_mean_edu_occ <- agg_mean_edu_occupied(df_train_personas)

df_train_hogares <- df_train_hogares %>%
  dplyr::left_join(train_mean_edu_occ, by = "id")

# Aplicar al TEST: personas -> hogares
test_mean_edu_occ <- agg_mean_edu_occupied(df_test_personas)

df_test_hogares <- df_test_hogares %>%
  dplyr::left_join(test_mean_edu_occ, by = "id")
```
